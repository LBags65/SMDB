...
SMDB_Metadata <- read_excel("C:/Users/lbagd/OneDrive - University of Toronto/Soil Metabolome Database/Database Documents/Publication Materials/Soil Metabolome Database.xlsx", 
                                       sheet = "1. Metadata")



#Load in 2. Location Data here
SMDB_Locations <- read_excel("C:/Users/lbagd/OneDrive - University of Toronto/Soil Metabolome Database/Database Documents/Publication Materials/Soil Metabolome Database.xlsx", 
  sheet = "2. Location Data")

#Load in 3. Metabolites here
Soil_Metabolome_Database <- read_excel("C:/Users/lbagd/OneDrive - University of Toronto/Soil Metabolome Database/Database Documents/Publication Materials/Soil Metabolome Database.xlsx", 
...
Superclass_Distinct_subtitle <- "<span style='font-size:24pt; font-weight:normal; font-style:italic'>(n = 58, m = 4290)</span>"
...
Superclass_Distinct_Table <- Soil_Metabolome_Database %>%
  filter(Metabolite_Lipid == "Metabolite") %>%
  filter(!is.na(Superclass)) %>%
  filter(Superclass != "Unclassified") %>%
  distinct(INCHIKEY, .keep_all = TRUE) %>%
  ...
  rename(Distinct_Percent = Percent)
...
Superclass_All_subtitle <- "<span style='font-size:24pt; font-weight:normal; font-style:italic'>(n=58, m = 7681)</span>"
...
Superclass_All_Table <- Soil_Metabolome_Database %>%
  filter(Metabolite_Lipid == "Metabolite") %>%
  filter(!is.na(Superclass)) %>%
  group_by(Superclass) %>%
  summarise(Count = n(), .groups = "drop") %>%
  ...
  rename(All = Count) %>%
  rename(All_Percent = Percent)

Superclass_SMDB <- merge(Superclass_Distinct_Table,
                         Superclass_All_Table,
                         by = "Superclass",
                         all = TRUE)

Superclass_SMDB_bar <- Superclass_SMDB %>%
  select(-c(Distinct, All)) %>% 
  rename(`Unique Metabolites` = Distinct_Percent) %>%
  rename(`All Metabolites` = All_Percent) %>%
  pivot_longer(cols = `Unique Metabolites`:`All Metabolites`, names_to = "type")

ggplot(Superclass_SMDB_bar, aes(x = type, y = value, fill = Superclass, label = value)) +
  geom_col() + 
  scale_fill_manual(values = Superclass_colors) +
  theme_minimal() +
  labs(y = "Percent") +
  ...
  theme(
    ...
    axis.text.x   = element_text(size = 14, vjust = 1, color = "black"),
    axis.text.y   = element_text(size = 14, color = "black"),
    axis.title.y  = element_text(size = 18, margin = margin(r = 7)),
...
amino_acids <- Soil_Metabolome_Database %>%
  filter(subclass == "Amino acids, peptides, and analogues") %>%
  group_by(INCHIKEY) %>%
  summarise(Count = n(), name = first(Metabolite_Name))

carbs <- Soil_Metabolome_Database %>%
  filter(subclass == "Carbohydrates and carbohydrate conjugates") %>%
  group_by(INCHIKEY) %>%
  summarise(Count = n(), name = first(Metabolite_Name))

fatty_acids <- Soil_Metabolome_Database %>%
  filter(subclass == "Fatty acids and conjugates") %>%
  group_by(INCHIKEY) %>%
  summarise(Count = n(), name = first(Metabolite_Name))

gc_organooxygen <- Soil_Metabolome_Database %>%
  filter(Superclass == "Organic oxygen compounds") %>%
  filter(Method == "GC/MS") %>%
  group_by(INCHIKEY) %>%
  summarise(Count = n(), name = first(Metabolite_Name))
...
#1 - 350x400
{
  Confone <- Conf_Superclass %>%
    filter(Conf_level == "1") %>%
    mutate(Superclass = factor(Superclass, levels = unique(Superclass)))
  
  
  Confone_subtitle <- "<span style='font-size:24pt; font-weight:normal; font-style:italic'>(n = 17, m = 1549)</span>"
  
  ggplot(Confone, aes(x = "", y = Percent, fill = Superclass)) +
    geom_bar(stat = "identity") +
    coord_polar(theta = "y") +
    scale_fill_manual(values = Superclass_colors) +
    labs(
...
#2 - 350x400
{
  Conftwo <- Conf_Superclass %>%
    filter(Conf_level == "2") %>%
    mutate(Superclass = factor(Superclass, levels = unique(Superclass)))
  
  
  Conftwo_subtitle <- "<span style='font-size:24pt; font-weight:normal; font-style:italic'>(n = 46, m = 6025)</span>"
  
  ggplot(Conftwo, aes(x = "", y = Percent, fill = Superclass)) +
    geom_bar(stat = "identity") +
    coord_polar(theta = "y") +
    scale_fill_manual(values = Superclass_colors) +
    labs(
...
Conf_Table <- Confidences %>%
  ...
  ungroup() %>%
  mutate(Superclass = as.character(Superclass)) %>%
  pivot_wider(names_from = Conf_level, values_from = c(Count, Percent)) %>%
...
#Water 
{
Water <- Extractions_Superclass_Top_3 %>%
  filter(Solvent_Matrix == "Water") %>%
  mutate(Superclass = factor(Superclass, levels = unique(Superclass)))
  

Water_subtitle <- "<span style='font-size:24pt; font-weight:normal; font-style:italic'>(n = 15, m = 1854)</span>"

ggplot(Water, aes(x = "", y = Percent, fill = Superclass)) +
  geom_bar(stat = "identity") +
  coord_polar(theta = "y") +
  scale_fill_manual(values = Superclass_colors) +
  labs(
...
#MeOH/Water 350x400
{
  MeOH_Water <- Extractions_Superclass_Top_3 %>%
    filter(Solvent_Matrix == "MeOH/Water") %>%
    mutate(Superclass = factor(Superclass, levels = unique(Superclass)))
  
  
  MeOH_Water_subtitle <- "<span style='font-size:24pt; font-weight:normal; font-style:italic'>(n = 19, m = 2747)</span>"
  
  ggplot(MeOH_Water, aes(x = "", y = Percent, fill = Superclass)) +
    geom_bar(stat = "identity") +
    coord_polar(theta = "y") +
    scale_fill_manual(values = Superclass_colors) +
    labs(
...
#MeOH/IPA/Water 350x400
{
  MeOH_PA_Water <- Extractions_Superclass_Top_3 %>%
    filter(Solvent_Matrix == "MeOH/IPA/Water") %>%
    mutate(Superclass = factor(Superclass, levels = unique(Superclass)))
  
  
  MeOH_IPA_Water_subtitle <- "<span style='font-size:24pt; font-weight:normal; font-style:italic'>(n = 5, m = 690)</span>"
  
  ggplot(MeOH_PA_Water, aes(x = "", y = Percent, fill = Superclass)) +
    geom_bar(stat = "identity") +
    coord_polar(theta = "y") +
    scale_fill_manual(values = Superclass_colors) +
    labs(
      ...
      y = NULL,
      fill = "Superclass",
      title = "MeOH/IPA/Water",
      subtitle = MeOH_IPA_Water_subtitle
    ) +
    theme_void() +
    guides(
      fill = guide_legend(nrow = 5)
...
Extractions_Superclass_Top_3_Bar <- Extractions %>%
  filter(Solvent_Matrix %in% c("MeOH/Water", "Water", "MeOH/IPA/Water")) %>%
  filter(!is.na(Superclass)) %>%
  group_by(Solvent_Matrix, Superclass) %>%
  summarise(Count = n(), .groups = "drop_last") %>%
...
Extractions_Superclass_Top_3_Bar$Solvent_Matrix <-  factor(Extractions_Superclass_Top_3_Bar$Solvent_Matrix,
                                                           levels = c("MeOH/Water", "Water", "MeOH/IPA/Water"))

ggplot(Extractions_Superclass_Top_3_Bar, aes(x = Solvent_Matrix, y = Count, fill = Superclass, label = Count)) +
  geom_col() + 
  scale_fill_manual(values = Superclass_colors) +
  theme_minimal() +
  labs(y = "Entries") +
...
Extractions_Superclass_Top_3_Bar_Table <- Extractions %>%
  filter(Solvent_Matrix %in% c("MeOH/Water", "Water", "MeOH/IPA/Water")) %>%
  filter(!is.na(Superclass)) %>%
  group_by(Solvent_Matrix, Superclass) %>%
  summarise(Count = n(), .groups = "drop_last") %>%
  ...
  mutate(Superclass = as.character(Superclass)) %>%
  select(-c(Percent)) %>%
  pivot_wider(names_from = "Solvent_Matrix", values_from = "Count")
...
MS_Column_Superclass_Bar <- MS_Column %>%
  filter(Column %in% c("GC_5%_Diphenyl", "LC_C18", "LC_HILIC")) %>% #choose major ones
  filter(!is.na(Superclass)) %>%
  group_by(Column, Superclass) %>%
  summarise(Count = n(), .groups = "drop_last") %>%
...
MS_Column_Superclass_Bar$Column <- factor(MS_Column_Superclass_Bar$Column,
                        levels = c("GC_5%_Diphenyl", "LC_C18", "LC_HILIC"),
                        labels = c("GC (5% Phenyl)", "LC (C18)", "LC (HILIC)")
)


ggplot(MS_Column_Superclass_Bar, aes(x = Column, y = Count, fill = Superclass, label = Count)) +
  geom_col() + 
  scale_fill_manual(values = Superclass_colors) +
  theme_minimal() +
  labs(y = "Entries") +
...
MS_Column_Superclass_Table <- MS_Column %>%
  filter(Column %in% c("GC_5%_Diphenyl", "LC_C18", "LC_HILIC")) %>% #choose major ones
  filter(!is.na(Superclass)) %>%
  group_by(Column, Superclass) %>%
  summarise(Count = n(), .groups = "drop_last") %>%
...
#GC phenyl
{
  GC_phenyl <- MS_Column_Superclass %>%
    filter(Column == "GC_5%_Diphenyl") %>%
    mutate(Superclass = factor(Superclass, levels = unique(Superclass)))
  
  
  GC_phenyl_subtitle <- "<span style='font-size:24pt; font-weight:normal; font-style:italic'>(n = 21, m = 2062)</span>"
  
  ggplot(GC_phenyl, aes(x = "", y = Percent, fill = Superclass)) +
    geom_bar(stat = "identity") +
    coord_polar(theta = "y") +
    scale_fill_manual(values = Superclass_colors) +
    labs(
...
#LC C18
{
  LC_C18 <- MS_Column_Superclass %>%
    filter(Column == "LC_C18") %>%
    mutate(Superclass = factor(Superclass, levels = unique(Superclass)))
  
  
  LC_C18_subtitle <- "<span style='font-size:24pt; font-weight:normal; font-style:italic'>(n = 24, m = 3425)</span>"
  
  ggplot(LC_C18, aes(x = "", y = Percent, fill = Superclass)) +
    geom_bar(stat = "identity") +
    coord_polar(theta = "y") +
    scale_fill_manual(values = Superclass_colors) +
    labs(
...
#LC HILIC
{
  LC_HILIC <- MS_Column_Superclass %>%
    filter(Column == "LC_HILIC") %>%
    mutate(Superclass = factor(Superclass, levels = unique(Superclass)))
  
  
  LC_HILIC_subtitle <- "<span style='font-size:24pt; font-weight:normal; font-style:italic'>(n = 12, m = 1366)</span>"
  
  ggplot(LC_HILIC, aes(x = "", y = Percent, fill = Superclass)) +
    geom_bar(stat = "identity") +
    coord_polar(theta = "y") +
    scale_fill_manual(values = Superclass_colors) +
    labs(
...
{
  GCMS_EI <- Ion_Mode %>%
    filter(Method_Ion == "GC/MS_+") %>%
    mutate(Superclass = factor(Superclass, levels = unique(Superclass)))
  
  
  GCMS_EI_subtitle <- "<span style='font-size:24pt; font-weight:normal; font-style:italic'>(n = 23, m = 2304)</span>"
  
  ggplot(GCMS_EI, aes(x = "", y = Percent, fill = Superclass)) +
    geom_bar(stat = "identity") +
    coord_polar(theta = "y") +
    scale_fill_manual(values = Superclass_colors) +
    labs(
...
{
  LCMS_pos <- Ion_Mode %>%
    filter(Method_Ion == "LC/MS_+") %>%
    mutate(Superclass = factor(Superclass, levels = unique(Superclass)))
  
  
  LCMS_pos_subtitle <- "<span style='font-size:24pt; font-weight:normal; font-style:italic'>(n = 21, m = 2272)</span>"
  
  ggplot(LCMS_pos, aes(x = "", y = Percent, fill = Superclass)) +
    geom_bar(stat = "identity") +
    coord_polar(theta = "y") +
    scale_fill_manual(values = Superclass_colors) +
    labs(
...
#LCMS - 350x400
{
  ...
  LCMS_neg_subtitle <- "<span style='font-size:24pt; font-weight:normal; font-style:italic'>(n = 21, m = 1833)</span>"
  
  ggplot(LCMS_neg, aes(x = "", y = Percent, fill = Superclass)) +
    geom_bar(stat = "identity") +
    coord_polar(theta = "y") +
    scale_fill_manual(values = Superclass_colors) +
    labs(
...
Ion_Mode_Bar <- Soil_Metabolome_Database %>%
  filter(Metabolite_Lipid == "Metabolite") %>%
  filter(!is.na(Superclass)) %>%
  mutate(Method = if_else(Method == "LC/IMS/MS", "LC/MS", Method)) %>%
  filter(Method != "CE/MS") %>%
...
Ion_Mode_Bar$Method_Ion <-  factor(Ion_Mode_Bar$Method_Ion, levels = c("GC/MS_+", "LC/MS_+", "LC/MS_-"),
                               labels = c("GC/MS (+)", "LC/MS (+)", "LC/MS (-)"))

ggplot(Ion_Mode_Bar, aes(x = Method_Ion, y = Count, fill = Superclass, label = Count)) +
  geom_col() + 
  scale_fill_manual(values = Superclass_colors) +
  theme_minimal() +
  labs(y = "Entries") +
...
Ion_Mode_Bar_Table <- Ion_Mode_Bar %>%
  select(-c(Percent)) %>%
  pivot_wider(names_from = "Method_Ion", values_from = "Count")

Extraction_Instrumentation_Table <- merge(Extractions_Superclass_Top_3_Bar_Table,
                                               MS_Column_Superclass_Table,
                                               by = "Superclass",
                                          all = TRUE) %>%
  merge(Ion_Mode_Bar_Table, by = "Superclass", all = TRUE) %>%
  mutate(across(everything(), ~ replace_na(.x, 0)))
...
top_inchi <- InChIKeY_DOI %>%
  ...
  merge(unique_metabolites[27:57], by = "INCHIKEY")

top_inchi_table <-  InChIKeY_DOI %>%
  filter(percentage > 10) %>% 
  merge(unique_metabolites, by = "INCHIKEY") %>%
  arrange(rank)

write_clip(top_inchi_table)
